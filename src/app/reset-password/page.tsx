...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    if (!token) {
      setValidationError("Thiếu token khôi phục mật khẩu. Vui lòng kiểm tra lại liên kết được gửi.");
    }
  }, [token]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!token) { setValidationError("Không tìm thấy mã token hợp lệ."); return; }
    if (!password || !confirmPassword) { setValidationError("Vui lòng nhập đầy đủ các trường."); return; }
    if (password.length < 6) { setValidationError("Mật khẩu mới phải dài tối thiểu 6 ký tự."); return; }
    if (password !== confirmPassword) { setValidationError("Xác nhận mật khẩu mới không khớp."); return; }
    ...
    try {
      ...
      if (!res.ok || !data.success) throw new Error(data.error || "Đặt lại mật khẩu thất bại.");
  ...
  return (
    <div className="bg-slate-900/40 backdrop-blur-xl py-8 px-4 border border-slate-800/80 shadow-2xl rounded-2xl sm:px-10">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="••••••••"
              className="w-full px-4 py-3 bg-slate-950 border border-slate-800 focus:border-[#EE4D2D] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#EE4D2D] text-slate-200 text-sm pr-12 transition-all disabled:opacity-50"
            />
            <button type="button" disabled={!token} onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-300 transition-colors disabled:opacity-50">
        ...
        <div>
          ...
          <input
            ...
            placeholder="••••••••"
            className="w-full px-4 py-3 bg-slate-950 border border-slate-800 focus:border-[#EE4D2D] rounded-xl focus:outline-none focus:ring-1 focus:ring-[#EE4D2D] text-slate-200 text-sm transition-all disabled:opacity-50"
          />
        </div>

        {validationError && (
          <div className="flex items-start space-x-2.5 p-4 bg-red-950/40 border border-red-900/50 rounded-xl text-red-300 text-xs">
            <AlertCircle className="w-5 h-5 shrink-0" /><p>{validationError}</p>
          </div>
        )}
        {error && (
          <div className="flex items-start space-x-2.5 p-4 bg-red-950/40 border border-red-900/50 rounded-xl text-red-300 text-xs">
        ...
        {success && (
          <div className="p-4 bg-emerald-950/40 border border-emerald-900/50 rounded-xl text-emerald-300 text-xs font-medium flex items-center">
            <span className="w-2 h-2 rounded-full bg-emerald-500 mr-2.5 animate-ping" />{success}
          </div>
        )}
      ...
      <div className="mt-6 pt-6 border-t border-slate-800/60 text-center">
        <Link href="/login" className="text-xs font-semibold text-[#EE4D2D] hover:text-[#d84022] transition-colors">
          Quay lại đăng nhập
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden font-sans">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md text-center space-y-4">
        ...
        <p className="text-sm text-slate-400">Nhập mật khẩu mới để hoàn tất quá trình khôi phục</p>
      </div>
